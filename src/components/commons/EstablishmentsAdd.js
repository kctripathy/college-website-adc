...
export default function EstablishmentsAdd({ title, code, description }) {
    ...
    const { EstbID, EstbTypeCode, EstbTypeCodeDescription, EstbDate, EstbTitle, EstbDescription, EstbShouldUpload } = values;
    ...
    const handleChangeFormField = (e) => {
        ...
        console.log("form id=", e.target.id, "form value = ", formValue)
        setValues({
            ...values,
            [e.target.id]: ((e.target.id === 'EstbShouldUpload') ? e.target.checked : formValue)
        })
    }
    ...
    const submitFormHandler = (e) => {
        ...
        if (validateFormFields()) {
            ...
            addEstablishment(UserID, { EstbID, EstbTypeCode, EstbDate, EstbTitle, EstbDescription, EstbStatusFlag: 'P' })
                .then(data => {
                    //debugger;
                    //console.log(data);
                    if (Number(data.status.code) > 0) {
                        ...
                        setError('');
                        //setValues({ ...values, EstbID: Number(data.status.code) })
            ...
            <form onSubmit={submitFormHandler} className="col-lg-12 col-sm-12 m-0 p-0 mt-4 w-100 text-left">
                ...
                <div className="form-group">
                    ...
                    <input
                        ...
                        id="EstbDate"
                        value={EstbDate}
                        onChange={handleChangeFormField} required />
                </div>
                {(code === 'R' || code === 'P') ? (<EstablishmentTypeCodes code={EstbTypeCode} description={description} handleChangeFormField={(e) => handleChangeFormField(e)} />) : ("")}
                ...
                <div className="form-group mt-2">
                    ...
                    <label htmlFor="EstbShouldUpload">{`I want to upload a file for this ${EstbTypeCodeDescription}`}  </label>
                </div>
                <div className="form-group text-center">
                    <button type="submit" className="btn btn-primary button-width">
                        <i className="fas fa-save mr-2"></i> SUBMIT
                    </button>
                    <button type="button" className="btn btn-primary ml-4 button-width" onClick={resetFormFileds}>
                    ...
                    <div className="alert alert-warning mt-2 text-left">
                        ...
                        <ul>
                            <li>
                                <em>The items marked as <span style={{ color: "red" }}>*</span> are required fields. You can't leave it blank</em>
                            </li>
                            <li>
                                <em>The <b>{EstbTypeCodeDescription}</b> will be displayed in website after approval by the Admin</em>
                            </li>
                            <li>
                                <em>You can only view the items (<b>{EstbTypeCodeDescription}</b>) added by you</em>
                            </li>
                            <li>
                                <em>Pending items (<b>{EstbTypeCodeDescription}</b>) are only editable. Approved items are not.</em>
                            </li>
                        </ul>
                    </div>
                </div>
