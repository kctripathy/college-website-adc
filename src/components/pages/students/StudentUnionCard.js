...
export default function StudentUnionCard(props) {
  const { year, election_date, description } = props.portfolio;
        ...
        <div className="col-lg-12 col-sm-12 text-left pl-4 pt-2">
          <p>
            {election_date !== null ? (
              <p
                style={{ display: "none" }}
              >{`Date of Election: ${election_date} `}</p>
            ) : (
              ""
            )}
            {description}
            ...
            {props.portfolio &&
              Object.keys(props.portfolio).map((key) => {
                ...
                if (
                  ...
                  return (
                    <div className="card m-2 p-0 w-100">
                      <div className="card-header m-0 p-1">
                        <div className="row m-0 p-0">
                          <div className="col-12 m-0 p-1">
                            <img
                              src={`../images/students_union/${year}/${photo}`}
                      ...
                      <div
                        className="card-footer tex-center m-0 p-0"
                        style={{ backgroundColor: "#ffffff" }}
                      >
                        <span className="portfolio-name">
                          {student_name &&
                            titleCase(student_name.replace(/_/g, " "))}
                        </span>
