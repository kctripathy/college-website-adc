                ...
                <>
                    <Link to="/gallery/photo">
                        ...
                        <div className="col-12 p-1 m-0 text-center text-dark bg-light border border-rounded">
                            <b>Latest Photo in Gallery</b>
                            <br />
                            <small className="text-dark">
                                {moment(state.latest_image.EstbDate).format("DD MMM YYYY HH:MM")}
                                <em>  ({moment(state.latest_image.EstbDate).fromNow()}</em>)
                        </small>
                        </div>
                        <div className="col-12 p-0 m-0" style={{ maxHeight: "250px", overflow: "hidden" }}>
                            <img src={`${WEB_URL}/Documents/${state.latest_image.FileNameWithPath}`} className="img img-responsive w-100" alt="" />
                        </div>
                    </Link>
                    {/* </a> */}
                </>
