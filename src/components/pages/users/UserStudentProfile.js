...
export default function UserStudentProfile(props) {
  ...
  const {
    ...
    LandPhoneNumber,
    Address_Present_TownOrCity,
    Address_Present_DistrictID,
    Address_Present_Pincode,
  } = values;

  useEffect(() => {
    loadAllDistricts().then((response) => {
      setDistricts(response);
    });
    if (state.students && state.students.length === 0) {
      dispatch(fetchStudents());
    } else {
      const arr = state.students.filter(
        (s) => s.StudentCode === props.UserName
      );
      setStudent(arr[0]);
      setValues({
        ...values,
  ...
  const validateFormFields = () => {
    ...
    if (MobileNumber && MobileNumber.length > 0 && MobileNumber.length !== 10) {
      ...
      LandPhoneNumber &&
      LandPhoneNumber.length > 0 &&
      LandPhoneNumber.length !== 10
    ) {
      isSuccess = false;
      message = "Phone Number should be 10 digit";
    } else if (
      Address_Present_Pincode &&
      Address_Present_Pincode.length > 0 &&
      Address_Present_Pincode.length !== 6
    ) {
      isSuccess = false;
  ...
  const showProfileInfo = () => {
    if (state.loading) return <Loading text="Fetching profile information" />;
    else {
      //
    ...
    return districts.length === 0 ? (
      ...
      <Fragment>
        ...
        <div className="col-5  p-1 w-100">
          <select
            className="w-100"
            name="Address_Present_DistrictID"
            value={Address_Present_DistrictID}
