...
function LayoutDashboardAdmin(props) {
  ...
  const fetchStaffProfile = () => {
    const theProfile =
      staff &&
      staff.staffs &&
      staff.staffs.filter((s) => s.EmployeeCode === UserName);
  ...
  const showProfilePhoto = () => {
    let profilePhotoUrl;
    if (RoleID === 1 || RoleID === 2 || RoleID === 3) {
      profilePhotoUrl = `${API_URL}/user/photo/employee/${UserReferenceID}/${Math.random()
        .toString(36)
        .slice(2)}`;
    } else if (RoleID === 4) {
      profilePhotoUrl = `${API_URL}/user/photo/student/${UserReferenceID}/${Math.random()
        .toString(36)
        .slice(2)}`;
    }
    return (
      <Fragment>
        <li
          ...
          style={{ display: RoleID === 1 ? "none" : "block" }}
        >
          <img
            src={`${profilePhotoUrl}`}
            className="img img-responsive w-100"
            alt={`${UserReferenceName}`}
          />
        ...
        <li
          className={`list-group-item ${isActive("/user/profile")}`}
          style={{ display: RoleID === 1 ? "none" : "block" }}
        >
      ...
      <div className="row m-0 p-0 mt-2">
        <div className="col-lg-3 col-sm-12 col-xs-12 m-0 p-0 text-center">
          <ul className="list-group">
            ...
            <li
              className={`list-group-item ${isActive("/user/change-password")}`}
            >
